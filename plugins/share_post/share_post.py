...
def share_post(content):
    ...
    twitter_link = 'http://twitter.com/intent/tweet?text=%s&url=%s' % (title, url)
    reddit_link = 'http://www.reddit.com/submit?title=%s&url=%s' % (title, url)
    hacker_news_link = 'https://news.ycombinator.com/submitlink?u=%s&t=%s' % (title, url)
